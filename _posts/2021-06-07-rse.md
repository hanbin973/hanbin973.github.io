...
시계열 (time-series)이나 종단 (longitudinal) 데이터의 경우 한 대상에서 복수의 관찰을 얻는데 이 관찰들을 서로 의존적입니다
반대로 다른 대상에서 온 관찰들은 독립임을 가정할 수 있어서 약간의 변형을 통해 GMM을 적용할 수 있습니다.
총 $n$개의 대상을 $T$번 관찰했을 경우 확률변수 $\mathbf{y}_i = \left[ y_{i1} \, \ldots \, y_{iT} \right]$
를 정의하겠습니다.
각 $y_i$는 독립이고 일변수 확률변수였던 이전의 경우와 다르게 벡터 확률변수가 되었음을 알 수 있습니다.

GMM을 적용하기 위해 $g:\mathbb{R}^T \times \mathbb{R}^p \rightarrow \mathbb{R}^T$를 다음과 같이 정의하겠습니다.

$$
g(\mathbf{y}, \boldsymbol{\beta}) = \left[X_{it} (y_{it} - \mathrm{exp}(X_{it}^T\boldsymbol{\beta}) \right]
$$
