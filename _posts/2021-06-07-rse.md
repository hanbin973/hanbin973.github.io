...
$W= \mathbf{I}$을 택하고 GMM의 objective function에 대입하면 objective function이 OLS 그 자체가 됨을 알 수 있습니다.
...
\left( 
	\frac{1}{n} \sum_{i=1}^n (y_i - X_i^T \boldsymbol{\beta})^T X^T
\right)
\left( 
	\frac{1}{n} \sum_{i=1}^n (y_i - X_i^T \boldsymbol{\beta}) X
\right)
