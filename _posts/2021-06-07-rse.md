...
이때 어떤 함수 $g:\mathbb{R}^q \times \mathbb{R}^p \rightarrow \mathbb{R}^q$가 존재해서 
...
\left(
\mathbb{E} \left[
\mathbf{X}_t^T \Sigma_t \mathbf{X}_t
\right]^{-1}
\right)_{t=1\ldots T}
$$
...
$$ G^TWG = \sum_{t=1}^T \mathbf{X}_t^T \Sigma_t \mathbf{X}_t $$

로 단순화되기 때문입니다.

지금까지 계산 결과를 종합하여 점근분포를 계산하면 다음을 얻습니다.

$$ \sqrt{n} \left(\boldsymbol{\hat{\beta}} - \boldsymbol{\beta}) \rightarrow
\mathcal{N} \left[0, \left( \sum_{t=1}^T \mathbf{X}_t^T \Sigma_t \mathbf{X}_t    \right)^-1     


\left( \sum_{t=1}^T \mathbf{X}_t^T \Sigma_t \mathbf{X}_t    \right)^-1
\right]
$$
