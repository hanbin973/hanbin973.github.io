...
이때 어떤 함수 $g:\mathbb{R}^n \times \mathbb{R}^p \rightarrow \mathbb{R}^q$가 존재해서 

$$ m(\theta_0) = \mathbb{E} \left[g(Y, \theta_0) \right] =0 $$

를 만족할 때를 생각하겠습니다.
대충 말하면 주어진 데이터 $Y \in \mathbb{R}^n$에 대해 실제 모수 $\theta_0$가 어떤 함수적 조건을 만족해야함을 뜻합니다.

이때 GMM 추정량은 다음으로 주어집니다.

$$ \hat{\theta} = \mathrm{argmin}_{\theta \in \mathbb{R}^p} 
\left( \frac{1}{n} \sum_{i=1}^n g(Y,\theta) \right)^T W
\left( \frac{1}{n} \sum_{i=1}^n g(Y,\theta) \right)
$$
...
를 생각합시다.
Homoskedastic linear normal model은 $\boldsymbol{\epsilon} \sim \mathcal{N}(0, \sigma^2 \mathbf{I})$과 $\boldsymbol{\epsilon} \perp \mathbf{X}$를 가정하지만 
OLS를 GMM으로 얻기 위해서는 더 약한 조건인 $\mathbb{E}[\mathbf{X}^T\boldsymbol{\epsilon}] = 0$만 있으면 충분합니다.

$$ g(Y,\theta) = g(\mathbf{y}, \boldsymbol{\beta}) = \mathbf{X}^T(\mathbf{y}-\mathbf{X}\boldsymbol{\beta}) $$

를 생각하면 위의 조건으로부터 $\mathbb{E} g(Y, \theta) = 0$이 됨을 확인할 수 있습니다.
$W= (\mathbf{X}^T \mathbf{X})^{-1}$을 택하고 GMM의 objective function에 대입하면 objective function이 OLS 그 자체가 됨을 알 수 있습니다.

$$ G = \nabla_{\theta} g(Y,\theta) = - \mathbf{X}^T\mathbf{X} $$

$$ \Sigma = \mathbf{X}^T \mathbb{E}\left[  (\mathbf{y}-\mathbf{X}\boldsymbol{\beta}) (\mathbf{y}-\mathbf{X}\boldsymbol{\beta})^T \right] \mathbf{X} $$

$$ \hat{\boldsymbol{\beta}} = \mathrm{argmin}_{\boldsymbol{\beta} \in \mathbb{R}^p} \left(\mathbf{y}-\mathbf{X} \boldsymbol{\beta} \right)^T \left(\mathbf{y}-\mathbf{X} \boldsymbol{\beta} \right) $$
...
\left[0,
	(\mathbf{X}^T\mathbf{X})^{-1} \Sigma  (\mathbf{X}^T\mathbf{X})^{-1}
\right]
$$
...
이 성립합니다.
Poisson GMM은 이 관찰로부터 $g(Y, \theta) = g(y, \beta) = X(y-\mathrm{exp}(X\beta))$를 이용해 추정을 시행합니다.
