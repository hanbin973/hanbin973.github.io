...
OLS를 GMM으로 얻기 위해서는 더 약한 조건인 $\mathbb{E}[\mathbf{X}\boldsymbol{\epsilon}] = 0$만 있으면 충분합니다.

$$ g(Y,\theta) = g(\mathbf{y}, \boldsymbol{\beta}, \boldsymbol{\sigma}) = \mathbf{X}(\mathbf{y}-\mathbf{X}\boldsymbol{\beta}) $$
