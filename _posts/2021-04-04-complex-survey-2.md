...
가 됨을 보였습니다.
여기서 $1-\frac{n_c}{C}$는 유한모집단 보정계수 (finite-popultion correction factor, FPC)로 이용지침서에 있는 바로 그 값입니다.
대개의 경우 $\frac{n_c}{C}$가 작기 때문에 $fpc$를 무시할 수 있습니다.

국건영의 경우 2단계 집락표본설계 위에 층화표본설계가 더해진 구조이므로 층을 뜻하는 첨자 $h$를 쓰면 최종적인 분산의 추정량은 위 식을 $h$에 대해 더한
...
\frac{n_{ch}}{n_{ch}-1}
\sum_{c \in A_{1h}} [
(\sum_{i \in B_{ch}} w_{hci} y_{hci})
	- \frac{1}{n_{ch}}{\sum_{d \in A_{1h}}(\sum_{i \in B_{dh}} w_{hdi} y_{hdi})}
		]^2
$$
...
이제 우리는 국건영을 정복했습니다(?).
다음 글에서는 이 공식을 이용해 복합표본설계 하에서 선형회귀분석 (OLS)가 어떻게 되는지 알아보겠습니다.
