...
으로 쓸 수 있습니다 ($ |A|=n $).
복잡한 두 수식이 같음을 보일 때는 한 쪽에서 다른 쪽으로만 가려고 하지 말고 양 쪽에서 조금씩 변형시켜 같음을 보이는 게 편리합니다.
표기법을 바꾸는 과정에서 이용지침서의 식을 변형했으니 이번에는 우리의 추정량을 변형할 차례입니다.
...
$$ 
\begin{align}
&= \sum_{c \neq d \in A_1} (\frac{n-N}{n(N-1)}-1) \sum_{i \in B_c} \sum_{j \in B_d}
