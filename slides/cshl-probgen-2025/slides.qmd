...
- Why are entries of $\mathbf{u}$ independent?
...
- Effect of $4-5 = \boldsymbol{\beta}_4$
:::
...
\Rightarrow
\quad
\mathbf{y} = \mathbf{Z}\mathbf{u} + \boldsymbol{\varepsilon} 
$$
where $\mathbf{Z}$ is the individual-edge (=branch) design matrix and $\mathbf{u}$ is the branch effects
:::

::: {.fragment}
Recall that $\mathbf{u}_e$ of edge $e$ is the aggregate effect of potential mutations
:::

::: {.incremental}
- $\mathbf{u}_e$ is random because mutations are random
- $\mathbf{u}_e$'s distribution is determined according to the mutational process (e.g. Poisson)
- We can *prove* that the entries of $\mathbf{u}$ are mutually independent
:::
