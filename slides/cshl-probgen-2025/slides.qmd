...
- Strong collinearity of some columns in $\mathbf{G}$, or tight linkage disequilibrium (LD)
...
::: {.fragment .fade-in-then-out fragment-index=1 .center}
Heritability is 
$$
h_g^2 = \frac{\mathrm{Var}(\mathbf{g}_n)}{\mathrm{Var}(\mathbf{y}_n)}
$$
This applies to all individual $n \in \{1,\ldots,N\}$
:::
...
::: {.fragment}
Predicting polygenic scores with $\textsf{tslmm}$ or nodes
:::
