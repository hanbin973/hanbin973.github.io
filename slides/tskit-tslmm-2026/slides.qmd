...
## Genetic variance and mutational variance

::: {.fragment}
$\textsf{tslmm}$'s target parameter $\tau^2$ represents the contribution of *mutations*.
$$
\tau^2 = \frac{1}{\text{# variants}} \sum_p \beta_p^2 u_p
$$
where $\beta_p$ is the effect size and $u_p$ is the mutation rate.
:::

::: {.fragment}
The genetic variance, on the other hand, is a *random variable*
$$
V_G = \frac{1}{N}\sum_{i=1}^N(\mathbf{g}_i - \overline{\mathbf{g}})^2
$$
:::

::: {.fragment}
We show that the relationship is roughly
$$
\mathrm{E}[V_G] = \tau^2 \cdot \frac{\mathrm{tr}(\widetilde{\mathbf{B}})}{N}
$$
The deviation may vary depending on the demographic structure.
:::
...
::: {.r-stack}
::: {.fragment}
![](imgs/matvec_compare.svg){fig-align="center" height=500}
:::
:::

::: {.r-stack}
::: {.fragment .fade-in-then-out}
$\textsf{tskit}$ uses an exact algorithm for GRM-vector products
:::

::: {.fragment .fade-in-then-out}
$\textsf{arg-needle-lib}$ uses a Monte-Carlo approach
:::

::: {.fragment .fade-in-then-out}
$\textsf{tskit}$ is **10--100x faster** than $\textsf{arg-needle-lib}$ across varying mutation rates
:::
...
::: {.r-stack}
::: {.fragment}
![](imgs/runtime_boltlmm.svg){fig-align="center" height=500}
:::
:::

::: {.r-stack}
::: {.fragment .fade-in-then-out}
$\textsf{BOLT-LMM}$ runtime scales as $\mathcal{O}(N^{1.5}M)$
:::
...
::: {.r-stack}
::: {.fragment}
![](imgs/pedigree.svg){fig-align="center" height=600}
:::
:::
...
::: {.fragment}
Comparable to standard SNP-based GRM with all causal variants included
:::
:::
